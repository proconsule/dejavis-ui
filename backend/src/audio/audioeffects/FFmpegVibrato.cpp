...
namespace audio_utils {
    ...
    FFmpegVibrato::~FFmpegVibrato() { destroyGraph(); }

    bool FFmpegVibrato::Init() { return Init(Config{}); }
    bool FFmpegVibrato::Init(const Config& cfg) { destroyGraph(); return buildGraph(cfg); }
    bool FFmpegVibrato::Reconfigure(const Config& cfg) { destroyGraph(); return buildGraph(cfg); }

    void FFmpegVibrato::destroyGraph() {
    ...
    void FFmpegVibrato::ensureScratch(std::vector<float>& v, size_t n) {
        if (v.size() < n) v.resize(n);
    }

    bool FFmpegVibrato::buildGraph(const Config& cfg) {
        ...
        AVFilterContext* vibCtx = nullptr;
        char vibArgs[64];
        std::snprintf(vibArgs, sizeof(vibArgs), "f=%.3f:d=%.3f", cfg.frequency, cfg.depth);
        if (avfilter_graph_create_filter(&vibCtx, avfilter_get_by_name("vibrato"), "vib", vibArgs, nullptr, graph_) < 0) { destroyGraph(); return false; }
        ...
        if (avfilter_link(srcCtx_, 0, vibCtx, 0) < 0 || avfilter_link(vibCtx, 0, fmtCtx, 0) < 0 || avfilter_link(fmtCtx, 0, sinkCtx_, 0) < 0) { destroyGraph(); return false; }
    ...
    bool FFmpegVibrato::pushFrame(const float* left, const float* right, size_t numFrames) {
    ...
    void FFmpegVibrato::drainAvailable() {
    ...
    void FFmpegVibrato::ringPush(const float* data, size_t numFloats) {
    ...
    size_t FFmpegVibrato::ringPop(float* dst, size_t numFloats) {
    ...
    void FFmpegVibrato::ProcessBlockStereo(float* inOut, size_t numFrames, float pre_gain) {
    ...
    void FFmpegVibrato::ProcessBlockStereoPlanar(float* left, float* right, size_t numFrames, float pre_gain) {
        if (!srcCtx_ || !sinkCtx_) return;
