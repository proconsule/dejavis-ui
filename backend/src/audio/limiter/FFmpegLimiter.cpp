...
namespace audio_utils {
    ...
    bool FFmpegLimiter::buildGraph(const Config& cfg) {
        ...
        if (avfilter_graph_create_filter(&fmtCtx,
                ...
                "fmt", "sample_fmts=flt", nullptr, graph_) < 0) {
            destroyGraph();
            return false;
        }
        ...
        std::vector<float> silence(latencyFrames_ * 2, 0.0f);
        pushFrame(silence.data(), latencyFrames_);
        drainAvailable();
        ringRead_ = ringWrite_ = ringCount_ = 0;
    ...
    bool FFmpegLimiter::pushFrame(const float* data, size_t numFrames) {
        ...
        std::memcpy(inFrame_->data[0], data, numFrames * 2 * sizeof(float));
    ...
    void FFmpegLimiter::ProcessBlockStereo(float* inOut, size_t numFrames, float pre_gain) {
        ...
        // Applica eventuale pre-gain prima di mandare al limiter
        if (pre_gain != 1.0f) {
    ...
    void FFmpegLimiter::ProcessBlockStereoPlanar(float* left, float* right,
        ...
        if (pre_gain != 1.0f) {
            for (size_t i = 0; i < numFrames; ++i) {
                scratchPlanar_[i * 2]     = left[i]  * pre_gain;
                scratchPlanar_[i * 2 + 1] = right[i] * pre_gain;
            }
        } else {
            for (size_t i = 0; i < numFrames; ++i) {
                scratchPlanar_[i * 2]     = left[i];
                scratchPlanar_[i * 2 + 1] = right[i];
            }
        }

        pushFrame(scratchPlanar_.data(), numFrames);
        drainAvailable();

        const size_t got = ringPop(scratchPlanar_.data(), totalFloats);
