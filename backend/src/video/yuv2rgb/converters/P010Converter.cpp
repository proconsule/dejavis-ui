...
bool P010Converter::recordUploadFromVulkanImage(VkCommandBuffer cmd,
                                                IConverterSlot* slot,
                                                VkImage srcImg,
                                                ...
                                                uint32_t w, uint32_t h)
{
    ...
    // (a) AVVkFrame image -> TRANSFER_SRC (multiplane non-disjoint: aspect COLOR).
    ...
    yReg.bufferOffset      = 0;
    yReg.bufferRowLength   = (uint32_t)(s->strideY / 2);   // byte/2 = texel da 16-bit
    ...
    // (c) Piano UV (PLANE_1, R10X6G10X6 = 4 byte/texel). rowLength = strideUV/4.
    //     extent a meta' risoluzione (4:2:0).
    VkBufferImageCopy uvReg{};
    uvReg.bufferOffset      = 0;
    uvReg.bufferRowLength   = (uint32_t)(s->strideUV / 4); // byte/4 = coppia da 2x16-bit
    ...
    // (d) buffer: TRANSFER_WRITE -> SHADER_READ.
    yuvconv::bufferBarrier(cmd, s->bufY,
